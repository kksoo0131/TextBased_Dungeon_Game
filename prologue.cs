using System;
using System.Threading.Tasks;
using static Sound2;
using static Gradation;

class Program
{
    static async Task Main(string[] args)
    {

        Console.Clear();
        Console.Write("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n                                             ");
        string S1 = "방랑자들이여, 위대한 스파르타 왕국에 온 것을 환영하네! ";
   

        for (int i = 0; i <= 255; i += 8)
        {
            await Task.Delay(170);
            Console.Write($"\u001b[38;2;0;{i};255m{S1.Substring(i / 8, 1)}");
            if (S1.Substring(i / 8, 1) != " " || S1.Substring(i / 8, 1) == ",")
            {  
                Play("C:\\Users\\User\\source\\repos\\kksoo0131\\TextBased_Dungeon_Game\\.vscode\\Type.mp3", false, true);
            }
           
        }
  
        await Task.Delay(350);
        Console.Clear();
        Console.Write("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n                                             ");
        string S2 = "5년 전만 해도, 우리 왕국은 정말 평화로운 왕국이었다네, ";


        for (int i = 0; i <= 255; i += 8)
        {
            await Task.Delay(170);
            Console.Write($"\u001b[38;2;0;{i};255m{S2.Substring(i / 8, 1)}");
            if (S2.Substring(i / 8, 1) != " " || S2.Substring(i / 8, 1) == "." || S2.Substring(i / 8, 1) == ",")
            {
                Play("C:\\Users\\User\\source\\repos\\kksoo0131\\TextBased_Dungeon_Game\\.vscode\\Type.mp3", false, true);
            }

        }

        await Task.Delay(350);
        Console.Clear();
        Console.Write("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n                                          ");
        string S3 = "허나, 그 평화도 잠깐.... 우리 왕국에 무시무시한 마왕이 나타났지뭔가...";


        for (int i = 0; i <= 255; i += 6)
        {
            await Task.Delay(170);
            Console.Write($"\u001b[38;2;0;{i};255m{S3.Substring(i / 6, 1)}");
            if (S3.Substring(i / 6, 1) != " " || S3.Substring(i / 6, 1) == "." || S3.Substring(i / 6, 1) == ",")
            {
                Play("\\Users\\User\\source\\repos\\kksoo0131\\TextBased_Dungeon_Game\\.vscode\\Type.mp3", false, true);
            }

        }

        await Task.Delay(350);
        Console.Clear();
        Console.Write("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n                                      ");
        string S4 = "자네들이, 그 마왕을 처치하고 우리 왕국에 다시 평화를 되찾아 줬으면 하네!!! ";

        for (int i = 0; i <= 255; i += 6)
        {
            await Task.Delay(170);
            Console.Write($"\u001b[38;2;0;{i};255m{S4.Substring(i / 6, 1)}");
            if (S4.Substring(i / 6, 1) != " " || S4.Substring(i / 6, 1) == "!" || S4.Substring(i / 6, 1) == ",")
            {
                Play("\\Users\\User\\source\\repos\\kksoo0131\\TextBased_Dungeon_Game\\.vscode\\Type.mp3", false, true);
            }

        }

        await Task.Delay(350);
        Console.Clear();
        Console.Write("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n                                                          ");
        string S5 = "자네들에게 신의 가호가 깃들기를... ";

        for (int i = 0; i <= 255; i += 14)
        {
            await Task.Delay(170);
            Console.Write($"\u001b[38;2;0;{i};255m{S5.Substring(i / 14, 1)}");
            if (S5.Substring(i / 14, 1) != " " || S5.Substring(i / 14, 1) == "!" || S5.Substring(i / 14, 1) == ",")
            {
                Play("\\Users\\User\\source\\repos\\kksoo0131\\TextBased_Dungeon_Game\\.vscode\\Type.mp3", false, true);
            }

        }  

    }
}